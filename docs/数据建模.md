...
> ⚠️ **建模前先看**：[需求分析文档](需求分析文档.md) §8.4 已判定本模型中 **8 张表属于过量设计建议裁剪、1 张建议合并**（裁剪后约 27 张）。下表清单是"需求的完整视图"，不代表一期全部要建。
...
| — | `auth_push_record`、`auth_login_log`、`checkin_certificate`、`checkin_exception_guide`、`qa_ask_log`、`nav_favorite`、`life_service_info`、`content_learning_record`、`admin_behavior_event` | **新增**（补齐推送、审计、核销、看板数据源、生活信息、学习记录等空白）；另有 `life_repair_log`、`content_club`、`content_file`、`admin_role_scope` 四张亦为新增，已随上面的拆分行说明 —— **新增合计 13 张** |
